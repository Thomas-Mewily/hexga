...
impl<T, const CAP: usize> ArrayVec<T, CAP> {
    ...
    pub fn pop(&mut self) -> Option<T> {
        ...
        unsafe {
            Some(self.values.as_ptr().add(self.len).read().assume_init())
        }
    }
    ...
    pub fn remove(&mut self, index: usize) -> T {
        self.try_remove(index).unwrap_or_else(|| {
            panic!("remove index (is {}) should be < len (is {})", index, self.len())
        })
    }
    ...
    pub fn swap_remove(&mut self, index: usize) -> T {
        self.swap_pop(index)
            .unwrap_or_else(|| {
                panic!("Can't swap remove at idx {index}")
            })
    }
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            let elements = slice::from_raw_parts_mut(
                self.array.get_unchecked_ptr(index),
                len - index);
...
impl<T> CapacityFullError<T> {
    ...
    pub const fn new(element: T) -> CapacityFullError<T> {
        CapacityFullError {
            element: element,
        }
    }
    ...
    pub fn simplify(self) -> CapacityFullError {
        CapacityFullError { element: () }
    }
}
