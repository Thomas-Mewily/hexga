...
pub unsafe trait BijectionFn
{
    ...
    {
        self.to_target(src).expect("bijection fail")
    }
    #[inline(always)]
    #[track_caller]
    fn to_source_unchecked(&self, target: Self::Target) -> Self::Source
    {
        self.to_source(target).expect("bijection fail")
    }
}
...
pub struct BijectionIdentity<T>
{
    phantom: PhantomData<T>,
}
impl<T> Default for BijectionIdentity<T>
{
    fn default() -> Self { Self::new() }
}
impl<T> Clone for BijectionIdentity<T>
{
    fn clone(&self) -> Self
    {
        Self {
...
impl<T> PartialEq for BijectionIdentity<T>
{
    fn eq(&self, _: &Self) -> bool { true }
}
impl<T> Eq for BijectionIdentity<T> {}
impl<T> Hash for BijectionIdentity<T>
{
    fn hash<H: Hasher>(&self, _: &mut H) {}
}
impl<T> PartialOrd for BijectionIdentity<T>
{
    fn partial_cmp(&self, _: &Self) -> Option<Ordering> { Some(Ordering::Equal) }
}
impl<T> Ord for BijectionIdentity<T>
{
    fn cmp(&self, _: &Self) -> Ordering { Ordering::Equal }
}
impl<T> Debug for BijectionIdentity<T>
{
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result { f.write_str("Identity") }
}
...
pub struct BijectionInverse<B>
{
    pub bijection: B,
}

unsafe impl<B> BijectionFn for BijectionInverse<B>
where
    B: BijectionFn,
{
    ...
    fn to_source(&self, target: Self::Target) -> Option<Self::Source>
    {
        self.bijection.to_target(target)
    }
    #[inline(always)]
    fn to_target_unchecked(&self, src: Self::Source) -> Self::Target
    {
        self.bijection.to_source_unchecked(src)
    }
    #[inline(always)]
    fn to_source_unchecked(&self, target: Self::Target) -> Self::Source
    {
        self.bijection.to_target_unchecked(target)
    }
}
unsafe impl<B> TryBijectionFn for BijectionInverse<B>
where
    B: TryBijectionFn,
{
    ...
    fn try_to_target(&self, src: Self::Source) -> Result<Self::Target, Self::SourceToTargetError>
    {
        self.bijection.try_to_source(src)
    }
    type TargetToSrcError = B::SourceToTargetError;
    #[inline(always)]
    fn try_to_source(&self, target: Self::Target) -> Result<Self::Source, Self::TargetToSrcError>
    {
        self.bijection.try_to_target(target)
    }
}
    ...
    B2: BijectionFn<Source = B1::Target>,
{
    ...
    fn to_target(&self, src: Self::Source) -> Option<Self::Target>
    {
        self.target.to_target(self.source.to_target(src)?)
    }
    #[inline(always)]
    fn to_source(&self, target: Self::Target) -> Option<Self::Source>
    {
        self.source.to_source(self.target.to_source(target)?)
    }
    #[inline(always)]
    fn to_target_unchecked(&self, src: Self::Source) -> Self::Target
    {
        self.target
            .to_target_unchecked(self.source.to_target_unchecked(src))
    }
    #[inline(always)]
    fn to_source_unchecked(&self, target: Self::Target) -> Self::Source
    {
        self.source
            .to_source_unchecked(self.target.to_source_unchecked(target))
    }
}
    ...
    B2: BijectionFn<Source = B1::Target> + TryBijectionFn,
{
    ...
    fn try_to_target(&self, src: Self::Source) -> Result<Self::Target, Self::SourceToTargetError>
    {
        let idx = self
            .source
            .try_to_target(src)
            .map_err(|e| BijectionError::Bijection(e))?;
        self.target
            .try_to_target(idx)
            .map_err(|e| BijectionError::Container(e))
    }
...
impl<T> From<T> for BijectionRev<usize>
where
    T: Length,
{
    fn from(value: T) -> Self { Self { len: value.len() } }
}
impl<Idx> BijectionRev<Idx>
{
    pub fn new(len: Idx) -> Self { Self { len } }
}
unsafe impl<Idx> BijectionFn for BijectionRev<Idx>
where
    Idx: Copy + Sub<Idx, Output = Idx> + One,
{
    type Source = Idx;
    type Target = Idx;
    #[inline(always)]
    fn to_target(&self, src: Self::Source) -> Option<Self::Target>
    {
        Some(self.to_target_unchecked(src))
    }
    #[inline(always)]
    fn to_source(&self, target: Self::Target) -> Option<Self::Source>
    {
        Some(self.to_source_unchecked(target))
    }
    #[inline(always)]
    ...
    fn to_source_unchecked(&self, target: Self::Target) -> Self::Source
    {
        self.to_target_unchecked(target)
    }
}
unsafe impl<Idx> TryBijectionFn for BijectionRev<Idx>
where
    Idx: Copy + Sub<Idx, Output = Idx> + One,
{
    type SourceToTargetError = Never;
    ...
    Idx: Integer,
{
    ...
    fn to_target(&self, src: Self::Source) -> Option<Self::Target>
    {
        src.is_inside(self.size)
            .then(|| self.to_target_unchecked(src))
    }
    ...
    Idx: Integer,
{
    ...
    {
        self.to_target(src)
            .ok_or(IndexOutOfRange::new(src, zero()..self.size))
    }
    ...
    fn try_to_source(&self, target: Self::Target) -> Result<Self::Source, Self::TargetToSrcError>
    {
        self.to_source(target)
            .ok_or(IndexOutOfRange::new(target, zero()..self.size.area_usize()))
    }
}
