...
macro_rules! impl_composite_types_and_methods_and_constants_for_internal_type 
{
    ...
    ) => 
    {
        ...
        impl<T $(, $($generic_params),* )?> $trait_name $(< $($generic_params),* >)? for RectangleOf<T> 
            where 
            T : $trait_name $(< $($generic_params),* >)?
        {
            $(
                type $type_name = RectangleOf<T::$type_name>;
            )*
            ...
            $(
                const $constant_name: Self = RectangleOf::new(T::$constant_name, T::$constant_name);
            )*
        }
