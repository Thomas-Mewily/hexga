...
#[non_exhaustive]
#[derive(Clone)]
pub enum DenseGridError<Idx, const N : usize> where Idx : Integer
{
...
impl<Idx, const N : usize> Debug for DenseGridError<Idx, N> where Idx : Debug, Idx : Integer
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        match self {
            DenseGridError::NegativeSize(size) => write!(f, "Can't have a negative size {:?}", size),
            DenseGridError::WrongDimension(size, got) => write!(f, "Wrong dimension : expected {:?} elements for a {:?} grid but got {:?} elements", size.area_usize(), size, got),
            DenseGridError::ToBig(size) => write!(f, "The size {:?} is too big !", size),
        }
    }
}
impl<Idx, const N : usize> Display  for DenseGridError<Idx, N> where Idx : Debug, Idx : Integer
{
...
impl<C, B, T, Idx, const N: usize> From<DenseGrid<C,B,T,Idx,N>> for Bijection<C,B>
    ...
    Idx: Integer
{
    fn from(value: DenseGrid<C,B,T,Idx,N>) -> Self {
        value.bijection
    }
}
    ...
    Idx: Integer
{
    fn from_values<P>(size: P, values: C) -> Option<Self>
        where P: Into<Vector<Idx,N>>, B: From<Vector<Idx,N>>
    ...
    {
        let size = size.into();
        if *size.min_element() < Idx::ZERO { return Err(DenseGridError::NegativeSize(size)); }
        ...
        if area_size != values.len() { return Err(DenseGridError::WrongDimension(size, area_size)); }
        Ok(unsafe { Self::from_bijection_unchecked(Bijection::from_values_and_bijection(values, B::from(size))) })
    }
    ...
    Idx: Integer
{
    unsafe fn from_bijection_unchecked(bijection: Bijection<S,B>) -> Self {
        Self { bijection, phantom: PhantomData }
    }
}