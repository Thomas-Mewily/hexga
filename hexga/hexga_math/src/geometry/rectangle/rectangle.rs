...
{
    fn from(value: RectangleOf<T>) -> Self { (value.pos, value.size) }
}
    ...
    T: Number,
{
    ...
    where
        T: One + Mul<T, Output = T> + Add<T, Output = T>,
    {
        self.pos + self.size * Vector::<T, 2>::X
    }
    ...
    {
        self.pos + self.size * Vector::<T, 2>::Y
    }
    ...
    T: Number,
{
    ...
    where
        Vector<T, N>: HaveX<T>,
    {
        self.middle().with_x(self.right_value())
    }
    pub fn middle_left(&self) -> Vector<T, N>
    where
        Vector<T, N>: HaveX<T>,
    {
        self.middle().with_x(self.left_value())
    }
    ...
    where
        Vector<T, N>: HaveY<T>,
    {
        self.middle().with_y(self.up_value())
    }
    pub fn middle_down(&self) -> Vector<T, N>
    where
        Vector<T, N>: HaveY<T>,
    {
        self.middle().with_y(self.down_value())
    }
    ...
    where
        Vector<T, N>: HaveZ<T>,
    {
        self.middle().with_z(self.forward_value())
    }
    pub fn middle_backward(&self) -> Vector<T, N>
    where
        Vector<T, N>: HaveZ<T>,
    {
        self.middle().with_z(self.backward_value())
    }
    ...
    where
        Vector<T, N>: HaveW<T>,
    {
        self.middle().with_w(self.ana_value())
    }
    pub fn middle_kata(&self) -> Vector<T, N>
    where
        Vector<T, N>: HaveW<T>,
    {
        self.middle().with_w(self.kata_value())
    }
...
impl<T, const N: usize> SetSize<T, N> for Rectangle<T, N>
where
    T: Number,
{
    #[inline(always)]
    ...
    T: Primitive,
{
    pub fn split_axis(&self, nb: T, axis: usize) -> impl Iterator<Item = Self>
    {
        ...
        cuts.clone().zip(cuts.skip(1)).map(move |(x0, x1)| {
    ...
    T: Number,
{
    ...
    where
        Vector<T, N>: Clamp,
    {
        vector.clamp(self.bounds_min(), self.bounds_max())
    }
...
pub trait Crop<T, const N: usize>: GetRectangle<T, N> + Sized
where
    T: Number,
{
    ...
    {
        if margin_start.any(|a| a < zero()) || margin_end.any(|a| a < zero())
        {
            return None;
        }
        ...
        if removed_size.any_with(subrect.size(), |a, b| a > b)
        {
            return None;
        }
...
impl<T, const N: usize> Crop<T, N> for Rectangle<T, N>
where
    T: Number,
{
...
impl<T, const N: usize> GetPosition<T, N> for Rectangle<T, N>
where
    T: Number,
{
    #[inline(always)]
...
impl<T, const N: usize> SetPosition<T, N> for Rectangle<T, N>
where
    T: Number,
{
    #[inline(always)]
    ...
    T: Number,
{
    ...
    where
        T: Integer,
    {
        self.size().area_usize()
    }
}

pub trait GetRectangle<T, const N: usize>: GetPosition<T, N> + GetSize<T, N>
where
    T: Number,
{
    #[inline(always)]
    ...
    where
        ...
        Range<T>: IntoIterator,
    {
        self.bounds_min().w()..self.bounds_max().w()
    }
    ...
    where
        Vector<T, N>: HaveX<T>,
    {
        x >= self.bounds_min().x() && x < self.bounds_max().x()
    }
    #[inline(always)]
    ...
    where
        Vector<T, N>: HaveY<T>,
    {
        y >= self.bounds_min().y() && y < self.bounds_max().y()
    }
    #[inline(always)]
    ...
    where
        Vector<T, N>: HaveZ<T>,
    {
        z >= self.bounds_min().z() && z < self.bounds_max().z()
    }
    #[inline(always)]
    ...
    where
        Vector<T, N>: HaveW<T>,
    {
        w >= self.bounds_min().w() && w < self.bounds_max().w()
    }
    ...
    T: Number,
{
    fn set_size(&mut self, size: Vector<T, N>) -> &mut Self;
    #[inline(always)]
    fn resize(&mut self, size: Vector<T, N>) -> &mut Self { self.set_size(size) }
    ...
    #[inline(always)]
    fn with_size(mut self, size: Vector<T, N>) -> Self
    where
        Self: Sized,
    {
        self.set_size(size);
    ...
    #[inline(always)]
    fn with_size_x(mut self, x: T) -> Self
    where
        Vector<T, N>: HaveX<T>,
        Self: Sized,
    {
    ...
    #[inline(always)]
    fn with_size_y(mut self, y: T) -> Self
    where
        Vector<T, N>: HaveY<T>,
        Self: Sized,
    {
    ...
    #[inline(always)]
    fn with_size_z(mut self, z: T) -> Self
    where
        Vector<T, N>: HaveZ<T>,
        Self: Sized,
    {
    ...
    #[inline(always)]
    fn with_size_w(mut self, w: T) -> Self
    where
        Vector<T, N>: HaveW<T>,
        Self: Sized,
    {
...
where
    S: GetRectangle<T, N> + GetSize<T, N> + SetPosition<T, N> + SetSize<T, N>,
    T: Number,
{
}
...
impl<Idx, const N: usize> IterIndex<Idx, N> for Rectangle<Idx, N>
where
    Idx: Integer,
{
