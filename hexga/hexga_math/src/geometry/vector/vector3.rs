...
pub trait SplatCoord3: Sized + Copy
{
    fn splat3(self) -> Vector3<Self> { Vector3::splat(self) }
}
impl<T: Copy> SplatCoord3 for T {}

impl<T> From<(T, T, T)> for Vector3<T>
{
    fn from(value: (T, T, T)) -> Self { Vector3::new(value.0, value.1, value.2) }
}
impl<T> From<Vector3<T>> for (T, T, T)
{
    fn from(value: Vector3<T>) -> Self
    {
        let [x, y, z] = value.array;
        (x, y, z)
    }
}
...
impl<T> HaveXAndOne<T> for Vector3<T>
where
    T: One + Zero,
{
    const X: Self = Vector3::new(T::ONE, T::ZERO, T::ZERO);
}
impl<T> HaveXAndMinusOne<T> for Vector3<T>
where
    T: MinusOne + Zero,
{
    const MINUS_X: Self = Vector3::new(T::MINUS_ONE, T::ZERO, T::ZERO);
}
...
where
    T: One + Zero,
{
    const Y: Self = Vector3::new(T::ZERO, T::ONE, T::ZERO);
}
impl<T> HaveYAndMinusOne<T> for Vector3<T>
where
    T: MinusOne + Zero,
{
    const MINUS_Y: Self = Vector3::new(T::ZERO, T::MINUS_ONE, T::ZERO);
}
...
impl<T> HaveZAndOne<T> for Vector3<T>
where
    T: One + Zero,
{
    const Z: Self = Vector3::new(T::ZERO, T::ZERO, T::ONE);
}
impl<T> HaveZAndMinusOne<T> for Vector3<T>
where
    T: MinusOne + Zero,
{
    const MINUS_Z: Self = Vector3::new(T::ZERO, T::ZERO, T::MINUS_ONE);
}
