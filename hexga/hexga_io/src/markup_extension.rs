...
{
    fn to_ron (&self) -> IoResult<String> { ron::ser::to_string_pretty(&self, ron::ser::PrettyConfig::___()).map_err(|e| IoError::new_serialize::<Self>(Io::RON_EXTENSION, e.to_debug())) }
...
{
    fn to_json(&self) -> IoResult<String> { serde_json::ser::to_string_pretty(&self).map_err(|e| IoError::new_serialize::<Self>(Io::JSON_EXTENSION, e.to_debug())) }
...
{
    fn to_xml (&self) -> IoResult<String> { quick_xml::se::to_string(&self).map_err(|e| IoError::new_serialize::<Self>(Io::XML_EXTENSION, e.to_debug())) }
...
pub trait MarkupExtensionQuickBin : Sized + Serialize + for<'de> Deserialize<'de>
{
    ...
    fn to_quick_bin(&self) -> IoResult<Vec<u8>> { bincode::serialize(self).map_err(|e| IoError::new_serialize::<Self>(Io::QUICK_BIN_EXTENSION, e.to_debug())) }
