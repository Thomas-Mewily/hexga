...
pub trait ToQuickBin : Serialize
{
    ...
    fn to_quick_bin(&self) -> EncodeResult<Vec<u8>> { bincode::serialize(self).map_err(|e| EncodeError::markup::<Self>(Io::QUICK_BIN, e)) }
}
impl<T> ToQuickBin for T where T: Serialize {}

pub trait FromQuickBin : for<'de> Deserialize<'de>
{
    fn from_quick_bin_buf(quick_bin : &[u8]) -> EncodeResult<Self> { bincode::deserialize(quick_bin).map_err(|e| EncodeError::markup::<Self>(Io::QUICK_BIN, e)) }
    fn from_quick_bin_with_reader<R : Read>(reader : R) -> EncodeResult<Self> { bincode::deserialize_from(reader).map_err(|e| EncodeError::markup::<Self>(Io::QUICK_BIN, e)) }
}
