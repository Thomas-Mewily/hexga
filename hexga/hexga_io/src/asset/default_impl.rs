...
impl Save for str
{
    ...
    fn save_to_with_custom_extension(&self, path: &path, _extension: &extension, fs: &mut Fs) -> IoResult {
...
impl Load for String
{
    ...
    fn load_from_with_custom_extension(path: &path, _extension: &extension, fs: &mut Fs) -> IoResult<Self> {
        fs.read_str(path)
    }
}
