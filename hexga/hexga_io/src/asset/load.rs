...
pub trait LoadExtension: Load
{
    ...
    fn load_from_or_create<'a, F, Fs, P>(path: P, init: F, fs: &mut Fs) -> Self where F:FnOnce() -> Self, Self: Save, Fs: FsWrite, P: AsRefPath
    {
    ...
    fn load_from<Fs, P>(path: P, fs: &mut Fs) -> IoResult<Self> where Fs: FsRead, P: AsRefPath
    {
        let path = path.as_ref();
