...
impl<const N:usize> MeshBuilder<N>
{
    ...
    pub fn from_vertices_and_indices(vertices: Vec<Vertex<N>>, indices: Vec<VertexIndex>) -> Self
    {
    ...
    pub fn to_vertices_and_triangles_indices(self) -> (Vec<Vertex<N>>, Vec<TriangleVertexIndex>) { (self.vertices, self.indices.chunks_exact(3).map(|c| TriangleVertexIndex::new(c[0], c[1], c[2])).collect()) }
