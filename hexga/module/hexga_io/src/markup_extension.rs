...
pub trait MarkupExtensionRon : Sized + Serialize + for<'de> Deserialize<'de>
{
    ...
    fn from_ron_buf<B : BufRead>(ron : B) -> IoResult<Self> { ron::de::from_reader(ron).map_err(|e| IoErrorKind::deserialize::<Self>(Io::RON_EXTENSION, e.to_debug()))  }
...
pub trait MarkupExtensionJson : Sized + Serialize + for<'de> Deserialize<'de>
{
    ...
    fn from_json_buf<B : BufRead>(json : B) -> IoResult<Self> { serde_json::de::from_reader(json).map_err(|e| IoErrorKind::deserialize::<Self>(Io::JSON_EXTENSION, e.to_debug())) }
...
pub trait MarkupExtensionXml : Sized + Serialize + for<'de> Deserialize<'de>
{
    ...
    fn from_xml_buf<B : BufRead>(xml : B) -> IoResult<Self> { quick_xml::de::from_reader(xml).map_err(|e| IoErrorKind::deserialize::<Self>(Io::XML_EXTENSION, e.to_debug())) }
