...
pub unsafe trait AllocFromLayout<Layout=AllocLayout>
{
    ...
    fn allocate_layout_or_panic(&mut self, layout: Layout) -> Self::Output { self.allocate_layout(layout).expect("bad alloc") }
}
...
pub trait Alloc<T> : ManagedBox
{
    ...
    fn allocate_or_panic(&mut self, value: T) -> Self::Box<T> { self.allocate(value).expect("bad alloc") }
}
impl<T,S> Alloc<T> for S where S: ManagedBox + AllocFromLayout<Output = AllocOutput>
{
    fn allocate(&mut self, value: T) -> AllocResult<Self::Box<T>> {
...
pub unsafe trait DeallocFromLayout<Layout = AllocLayout, Ptr = NonNullUnaliased<u8>> : AllocFromLayout<Layout>
{
    ...
    fn realloc_layout_or_panic(&mut self, ptr: Ptr, old_layout: Layout, new_layout: Layout) -> Self::Output { self.realloc_layout(ptr, old_layout, new_layout).expect("bad realloc") }
}
...
unsafe impl DeallocFromLayout for Memory {
    ...
    fn realloc_layout(
        ...
        let new_ptr = unsafe { std::alloc::realloc(ptr.as_ptr() as *mut u8, old_layout, new_layout.size()) };
        if new_ptr.is_null() {
            Err(AllocError)
        } else {
            Ok(NonNullUnaliased::slice_from_raw_parts(
