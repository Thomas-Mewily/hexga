...
pub trait CollectExtension<T> : Sized + Iterator<Item = T>
{
...
impl<I,T> CollectExtension<T> for I where I : Iterator<Item = T> {}

pub trait CollectMapExtension<T1,T2> : Sized + Iterator<Item = (T1,T2)>
{
...
impl<I,T1,T2> CollectMapExtension<T1,T2> for I where I : Iterator<Item = (T1,T2)> {}