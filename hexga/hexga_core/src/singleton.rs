use super::*;
use std::{cell::LazyCell, sync::{LazyLock, MappedMutexGuard, MappedRwLockReadGuard, MappedRwLockWriteGuard, Mutex, PoisonError, RwLock, RwLockReadGuard, RwLockWriteGuard, TryLockError}};
...
pub mod prelude
{
    pub use super::{Singleton,SingletonSingleThread,SingletonRwLock,SingletonMutex};
}
...
impl<T> Debug for SingletonOf<T> where T: Debug
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result { write!(f, "Singleton({:?})", self.inner) }
}
impl<T> Deref for SingletonOf<T> where T: Deref
{
...
impl<T> DerefMut for SingletonOf<T> where T: DerefMut
{
    #[inline(always)]
    #[track_caller]
...
pub type SingletonSingleThreadReadResultOf<'a,T> = Result<SingletonSingleThreadReadOf<'a,T>,SingletonSingleThreadReadError>;
...
pub type SingletonSingleThreadWriteResultOf<'a,T> = Result<SingletonSingleThreadWriteOf<'a,T>, SingletonSingleThreadWriteError>;
...
{
    type Guard<'a> = SingletonSingleThreadReadOf<'a,T> where Self: 'a;
    fn get<'a>(&'a self) -> Self::Guard<'a> { SingletonSingleThreadReadOf::new(self.inner.get().guard_map(Deref::deref)) }
}
impl<T> TryGuarded<T> for SingletonSingleThread<T>
{
    ...
    {
        Ok(SingletonSingleThreadReadOf::new(self.inner.try_get()?.guard_map(Deref::deref)))
    }
}
impl<T> GuardedMut<T> for SingletonSingleThread<T>
{
    type GuardMut<'a> = SingletonSingleThreadWriteOf<'a,T> where Self: 'a;
    fn get_mut<'a>(&'a self) -> Self::GuardMut<'a> { SingletonSingleThreadWriteOf::new(self.inner.get_mut().guard_map_mut(DerefMut::deref_mut)) }
}
impl<T> TryGuardedMut<T> for SingletonSingleThread<T>
{
    ...
    {
        Ok(SingletonSingleThreadWriteOf::new(self.inner.try_get_mut()?.guard_map_mut(DerefMut::deref_mut)))
    }
}
...
pub type SingletonRwLockReadResultOf<'a,T> = Result<SingletonRwLockReadOf<'a,T>,SingletonRwLockReadErrorOf<'a,T>>;
...
pub type SingletonRwLockWriteResultOf<'a,T> = Result<SingletonRwLockWriteOf<'a,T>,SingletonRwLockWriteErrorOf<'a,T>>;
...
{
    type Guard<'a> = SingletonRwLockReadOf<'a,T> where Self: 'a;
    fn get<'a>(&'a self) -> Self::Guard<'a> { SingletonRwLockReadOf::new(self.inner.get().guard_map(Deref::deref)) }
}
impl<T> TryGuarded<T> for SingletonRwLock<T>
{
    type Error<'a> = SingletonRwLockReadErrorOf<'a, T> where Self: 'a;
    fn try_get<'a>(&'a self) -> SingletonRwLockReadResultOf<'a, T> { Ok(SingletonRwLockReadOf::new(self.inner.try_get()?.guard_map(Deref::deref))) }
}
impl<T> GuardedMut<T> for SingletonRwLock<T>
{
    type GuardMut<'a> = SingletonRwLockWriteOf<'a,T> where Self: 'a;
    fn get_mut<'a>(&'a self) -> Self::GuardMut<'a> { SingletonRwLockWriteOf::new(self.inner.get_mut().guard_map_mut(DerefMut::deref_mut)) }
}
impl<T> TryGuardedMut<T> for SingletonRwLock<T>
{
    type Error<'a> = SingletonRwLockWriteErrorOf<'a,T> where Self: 'a;
    fn try_get_mut<'a>(&'a self) -> SingletonRwLockWriteResultOf<'a, T> { Ok(SingletonRwLockWriteOf::new(self.inner.try_get_mut()?.guard_map_mut(DerefMut::deref_mut))) }
}
...
pub type SingletonMutexWriteResultOf<'a,T> = Result<SingletonMutexWriteOf<'a,T>,SingletonMutexWriteErrorOf<'a,T>>;
...
{
    type GuardMut<'a> = SingletonMutexWriteOf<'a,T> where Self: 'a;
    fn get_mut<'a>(&'a self) -> Self::GuardMut<'a> { SingletonMutexWriteOf::new(self.inner.get_mut().guard_map_mut(DerefMut::deref_mut)) }
}
impl<T> TryGuardedMut<T> for SingletonMutex<T>
{
    type Error<'a> = SingletonMutexWriteErrorOf<'a,T> where Self: 'a;
    fn try_get_mut<'a>(&'a self) -> SingletonMutexWriteResultOf<'a, T> { Ok(SingletonMutexWriteOf::new(self.inner.try_get_mut()?.guard_map_mut(DerefMut::deref_mut))) }
}
...
impl<'a,D,T> Guard<'a,T> for SingletonGuard<D> where D: Guard<'a,T>
{
    type Mapped<U> = SingletonGuard<D::Mapped<U>> where U: 'a + ?Sized;

    fn guard_map<U, F>(self, f: F) -> Self::Mapped<U> where F: FnOnce(&T) -> &U, U: 'a + ?Sized {
        SingletonGuard::new(self.guard.guard_map(f))
    }
}
impl<'a,D,T> GuardMut<'a,T> for SingletonGuard<D> where D: GuardMut<'a,T>
{
    type MappedMut<U> = SingletonGuard<D::MappedMut<U>> where U: 'a + ?Sized;
