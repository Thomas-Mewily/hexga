...
{
    #[inline(always)]
    fn any<P>(self, p: P) -> bool where P: FnMut(Item) -> bool { self.into_iter().any(p) }
    #[inline(always)]
    fn all<P>(self, p: P) -> bool where P: FnMut(Item) -> bool { self.into_iter().all(p) }
    //#[inline(always)]
    //fn for_each<F>(self, f: F) where F: FnMut(Item) { self.into_iter().for_each(f); }

    #[inline(always)]
    fn any_with<P,O>(self, other: O, mut p: P) -> bool where P: FnMut(Item, Item) -> bool, O : IntoIterator<Item = Item>
    {
    ...
    #[inline(always)]
    fn all_with<P,O>(self, other: O, mut p: P) -> bool where P: FnMut(Item, Item) -> bool, O : IntoIterator<Item = Item>
    {
        let it_a = self.into_iter();
        let it_b = other.into_iter();
