...
impl<T> ExternLibConvert<Point2> for winit::dpi::LogicalSize<T> where T : ToInt<Output = int>
{
...
impl<T> ExternLibConvert<Point2> for winit::dpi::PhysicalSize<T> where T : ToInt<Output = int>
{
    fn convert(self) -> Point2 { point2(self.width.to_int(), self.height.to_int()) }
}
...
impl<T> ExternLibConvert<Point2> for winit::dpi::PhysicalPosition<T> where T : ToInt<Output = int>
{
