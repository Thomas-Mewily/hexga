    ...
    T: GpuBufferElement,
{
    ...
    pub fn update(&mut self, src: &[T]) { self.try_update(src).expect("bit cast failed") }
}
...
impl<'a, T> Length for GpuSliceMutWrite<'a, T>
where
    T: GpuBufferElement,
{
    fn len(&self) -> usize { self.view.len() }
}
