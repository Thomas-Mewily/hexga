...
pub trait WindowExtension:
    GetPosition<int, 2> + SetPosition<int, 2> + GetSize<int, 2> + SetSize<int, 2>
{
...
impl AppWindow
{
    pub(crate) fn new() -> Self
    {
        Self {
            window: None,
            param: winit::window::WindowAttributes::default(),
        }
    }
    ...
    pub(crate) fn init_if_needed(&mut self, active: &EventLoopActive) -> bool
    {
        ...
        let window = Arc::new(active.create_window(win_attr).expect("create window err."));
...
impl Window
{
    ...
    pub(crate) fn drop_surface(&mut self) { self.surface = None; }
...
{
    fn request_draw(&mut self) { self.window.request_redraw(); }
    fn configure_surface(&mut self)
    {
        let size = self.size();
...
impl<T> WindowParamBuilder for T
where
    T: HasMut<WindowParam>,
{
