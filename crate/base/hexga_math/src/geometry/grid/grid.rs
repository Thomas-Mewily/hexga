...
pub struct GridOf<T, Idx, const N: usize>
where
    Idx: Integer,
{
        ...
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result
        {
            ...
            match N
            {
                2 =>
                {
                    ...
                    let g = unsafe { GridOf::from_vec_unchecked(size, strings) };
...
map_on_std_fmt!(
    ($trait_name :ident) =>
    {
        impl<T, Idx, const N : usize> std::fmt::$trait_name for GridOf<T, Idx, N> where Idx : Integer, T: std::fmt::$trait_name
        {
...
impl<T, Idx, const N: usize> Map for GridOf<T, Idx, N>
where
    Idx: Integer,
{
    type WithType<R> = GridOf<R, Idx, N>;
...
impl<T, Idx, const N: usize> GridOf<T, Idx, N>
where
    Idx: Integer,
{
...
impl<T, Idx, const N: usize> IGrid<T, Idx, N> for GridOf<T, Idx, N>
where
    Idx: Integer,
{
    type WithType<U> = GridOf<U, Idx, N>;
    ...
    P: Into<Vector<Idx, N>>,
{
    ...
    {
        self.position_to_index(pos.into())
            .and_then(|idx| Some(unsafe { self.values().get_unchecked(idx) }))
    }
    #[inline(always)]
    #[track_caller]
...
impl<P, T, Idx, const N: usize> TryGet<P> for GridOf<T, Idx, N>
where
    Idx: Integer,
    P: Into<Vector<Idx, N>>,
{
...
impl<P, T, Idx, const N: usize> TryGetMut<P> for GridOf<T, Idx, N>
where
    Idx: Integer,
    P: Into<Vector<Idx, N>>,
{
    ...
    P: Into<Vector<Idx, N>>,
{
    ...
    {
        self.position_to_index(pos.into())
            .and_then(|i| Some(unsafe { self.values_mut().get_unchecked_mut(i) }))
    }
    #[track_caller]
    #[inline(always)]
...
impl<P, T, Idx, const N: usize> GetManyMut<P> for GridOf<T, Idx, N>
where
    Idx: Integer,
    P: Into<Vector<Idx, N>>,
{
...
impl<P, T, Idx, const N: usize> Index<P> for GridOf<T, Idx, N>
where
    Idx: Integer,
    P: Into<Vector<Idx, N>>,
{
...
impl<P, T, Idx, const N: usize> IndexMut<P> for GridOf<T, Idx, N>
where
    Idx: Integer,
    P: Into<Vector<Idx, N>>,
{
...
impl<T, Idx, const N: usize> Crop<Idx, N> for GridOf<T, Idx, N>
where
    Idx: Integer,
    T: Clone,
{
...
mod grid_test
{
    ...
    fn index_order()
    {
        ...
        */
        ...
        let mut indice = 0;
        assert_eq!(grid.values()[indice], grid[int2(0, 0)]);
        assert_eq!(grid.values()[indice], 0);

        indice += 1;
        assert_eq!(grid.values()[indice], grid[int2(1, 0)]);
        assert_eq!(grid.values()[indice], 1);

        indice += 1;
        assert_eq!(grid.values()[indice], grid[int2(0, 1)]);
        assert_eq!(grid.values()[indice], 10);

        indice += 1;
        assert_eq!(grid.values()[indice], grid[int2(1, 1)]);
        assert_eq!(grid.values()[indice], 11);

        indice += 1;
        assert_eq!(grid.values()[indice], grid[int2(0, 2)]);
        assert_eq!(grid.values()[indice], 20);

        indice += 1;
        assert_eq!(grid.values()[indice], grid[int2(1, 2)]);
    ...
    {
        use super::*;
        let grid = Grid2::from_fn(int2(2, 3), |_| 42);

        assert_eq!(grid.get(int2(0, 0)), Some(&42));
        assert_eq!(grid.get(int2(-1, 0)), None);
        assert_eq!(grid.get(int2(1, 0)), Some(&42));
        assert_eq!(grid.get(int2(2, 0)), None);

        assert_eq!(grid.get(int2(0, 0)), Some(&42));
        assert_eq!(grid.get(int2(0, -1)), None);
        assert_eq!(grid.get(int2(0, 2)), Some(&42));
        assert_eq!(grid.get(int2(0, 3)), None);
    }
    ...
    fn slice_cmp()
    {
        ...
        let top_right_grid_size = int2(1, 2);
        let offset = Vector2::ONE;
        let top_right_grid = Grid2::from_fn(smaller_size, |p| {
            let p = p + offset;
            p.x + 10 * p.y
        });
