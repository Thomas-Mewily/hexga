    ...
    Idx: Integer,
{
    ...
    fn fill_fn<F>(&mut self, mut f: F)
    where
        F: FnMut(Vector<Idx, N>) -> T,
    {
        self.for_each_mut(|(idx, v)| {
            *v = f(idx);
        })
    }
    ...
    Idx: Integer,
{
    ...
    where
        'a: 'b,
    {
        unsafe { GridView::from_rect_unchecked(self.grid_unchecked(), self.rect()) }
    }
    ...
    Idx: Integer,
{
        ...
        'a: 'b,
    {
        unsafe {
            let r = self.rect();
            GridViewMut::from_rect_unchecked(self.grid_mut_unchecked(), r)
        }
    }

    fn iter_mut(&mut self) -> GridViewIterMut<'_, G, T, Idx, N>
    {
        GridViewIterMut::from_rect(self.grid, self.rect()).unwrap()
    }
    fn for_each_mut<F>(&mut self, f: F)
    where
        F: FnMut((Vector<Idx, N>, &mut T)),
...
where
    ...
    T: PartialEq,
{
    fn eq(&self, other: &Self) -> bool { self.rect == other.rect && self.iter().eq(other.iter()) }
}
...
impl<'a, G, T, Idx, const N: usize> Collection for GridViewMut<'a, G, T, Idx, N>
where
    G: IGrid<T, Idx, N>,
    Idx: Integer,
{
    ...
    P: Into<Vector<Idx, N>>,
{
    ...
    {
        self.grid.try_get_mut(index.into() + self.rect.pos)
    }
}
...
{
    #[inline(always)]
    fn get_mut(&mut self, index: P) -> Option<&mut Self::Output>
    {
        self.grid.get_mut(index.into() + self.rect.pos)
    }
    #[inline(always)]
    #[track_caller]
    unsafe fn get_unchecked_mut(&mut self, index: P) -> &mut Self::Output
    {
        unsafe { self.grid.get_unchecked_mut(index.into() + self.rect.pos) }
    }
}
    ...
    P: Into<Vector<Idx, N>>,
{
    ...
    {
        self.grid
            .try_get_many_mut(indices.map(|idx| idx.into() + self.rect.pos))
    }
    #[inline(always)]
    ...
    {
        self.grid
            .get_many_mut(indices.map(|idx| idx.into() + self.rect.pos))
