...
pub trait SplatCoord4: Sized + Copy
{
    fn splat4(self) -> Vector4<Self> { Vector4::splat(self) }
}
impl<T: Copy> SplatCoord4 for T {}

impl<T> From<(T, T, T, T)> for Vector4<T>
{
    fn from(value: (T, T, T, T)) -> Self { Vector4::new(value.0, value.1, value.2, value.3) }
}
impl<T> From<Vector4<T>> for (T, T, T, T)
{
    fn from(value: Vector4<T>) -> Self
    {
        let [x, y, z, w] = value.array;
        (x, y, z, w)
    }
}
...
pub type Vec4U = VecU<4>;
pub const fn vec4u(x: uint, y: uint, z: uint, w: uint) -> Vec4U { Vec4U::new(x, y, z, w) }
...
impl<T> HaveXAndOne<T> for Vector4<T>
where
    T: One + Zero,
{
    const X: Self = Vector4::new(T::ONE, T::ZERO, T::ZERO, T::ZERO);
}
impl<T> HaveXAndMinusOne<T> for Vector4<T>
where
    T: MinusOne + Zero,
{
    const MINUS_X: Self = Vector4::new(T::MINUS_ONE, T::ZERO, T::ZERO, T::ZERO);
}
...
where
    T: One + Zero,
{
    const Y: Self = Vector4::new(T::ZERO, T::ONE, T::ZERO, T::ZERO);
}
impl<T> HaveYAndMinusOne<T> for Vector4<T>
where
    T: MinusOne + Zero,
{
    const MINUS_Y: Self = Vector4::new(T::ZERO, T::MINUS_ONE, T::ZERO, T::ZERO);
}

impl<T> HaveZ<T> for Vector4<T>
{
    fn iter_xyz<'a>(&'a self) -> impl Iterator<Item = &'a T>
    where
        T: 'a,
    {
    ...
    fn iter_xyz_mut<'a>(&'a mut self) -> impl Iterator<Item = &'a mut T>
    where
        T: 'a,
    {
...
impl<T> HaveZAndOne<T> for Vector4<T>
where
    T: One + Zero,
{
    const Z: Self = Vector4::new(T::ZERO, T::ZERO, T::ONE, T::ZERO);
}
impl<T> HaveZAndMinusOne<T> for Vector4<T>
where
    T: MinusOne + Zero,
{
    const MINUS_Z: Self = Vector4::new(T::ZERO, T::ZERO, T::MINUS_ONE, T::ZERO);
}
...
where
    T: One + Zero,
{
    const W: Self = Vector4::new(T::ZERO, T::ZERO, T::ZERO, T::ONE);
}
impl<T> HaveWAndMinusOne<T> for Vector4<T>
where
    T: MinusOne + Zero,
{
    const MINUS_W: Self = Vector4::new(T::ZERO, T::ZERO, T::ZERO, T::MINUS_ONE);
}
...
where
    T: Default,
{
    fn from(value: Vector2<T>) -> Self { value.resize() }
}
impl<T> From<Vector3<T>> for Vector4<T>
where
    T: Default,
{
    fn from(value: Vector3<T>) -> Self { value.resize() }
}
...
{
    pub use super::{Vec4B, Vec4I, SplatCoord4, Vec4U, Vec4, Vector4, vec4b, vec4i, vec4u, vec4, vector4};
}
