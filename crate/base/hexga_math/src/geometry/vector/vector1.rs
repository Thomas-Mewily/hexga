...
pub trait SplatCoord1: Sized + Copy
{
    fn splat1(self) -> Vector1<Self> { Vector1::splat(self) }
}
impl<T: Copy> SplatCoord1 for T {}

impl<T> From<(T,)> for Vector1<T>
{
    fn from(value: (T,)) -> Self { Vector1::new(value.0) }
}
impl<T> From<Vector1<T>> for (T,)
{
    fn from(value: Vector1<T>) -> Self
    {
        let [x] = value.array;
        (x,)
    }
...
impl<T> HaveXAndOne<T> for Vector1<T>
where
    T: One + Zero,
{
    const X: Self = Vector1::new(T::ONE);
}
impl<T> HaveXAndMinusOne<T> for Vector1<T>
where
    T: MinusOne + Zero,
{
    const MINUS_X: Self = Vector1::new(T::MINUS_ONE);
}
...
{
    pub use super::{Bool1, Point1, SplatCoord1, Vec1, Vector1, bool1, point1, vec1, vector1};
}
