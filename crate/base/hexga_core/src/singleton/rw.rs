...
{
    pub const fn new(value: T) -> Self { Self { guarded: RwLock::new(Identity::new(value)) }}
}
...
impl<T> SingletonLazyRw<T> {
    pub const fn new(f: fn() -> T) -> Self { 
        Self { guarded: RwLock::new(LazyLock::new(f)) }
    }
}
...
impl<T> Guarded<T> for SingletonOptionRw<T> {
    ...
    #[track_caller]
    fn get<'a>(&'a self) -> Self::Guard<'a> {
        let guard = self.guarded.read().unwrap_or_else(|e| {
...
impl<T> GuardedMut<T> for SingletonOptionRw<T> {
    ...
    #[track_caller]
    fn get_mut<'a>(&'a self) -> Self::GuardMut<'a> {
        let guard = self.guarded.write().unwrap_or_else(|e| {
