...
impl<T> Guarded<T> for SingletonCell<T>
{
    ...
    #[track_caller]
    fn get<'a>(&'a self) -> Self::Guard<'a> { Ref::map(self.guarded.get(), |v| v.as_ref()) }
...
impl<T> SingletonOptionCell<T>
{
    ...
    pub const fn new(value: Option<T>) -> Self { Self::from_guard(SingleThreadCell::new(value)) }
}
...
impl<T> Guarded<T> for SingletonOptionCell<T>
{
    ...
    {
        self.guarded.get().guard_map(|g| {
            g.as_ref().unwrap_or_else(|| match self.try_get()
            {
                Ok(_) => panic!(
                    "SingletonOptionCell<{}> can't be read",
                    std::any::type_name::<T>()
...
impl<T> GuardedMut<T> for SingletonOptionCell<T>
{
    ...
    fn get_mut<'a>(&'a self) -> Self::GuardMut<'a>
    {
        RefMut::filter_map(self.guarded.get_mut(), |opt| opt.as_mut()).unwrap_or_else(
            |_| match self.try_get()
            {
                Ok(_) => panic!(
