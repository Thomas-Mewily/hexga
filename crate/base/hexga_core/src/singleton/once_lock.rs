...
unsafe impl<T> core::marker::Sync for SingletonOnce<T> {}
#[cfg(all(target_arch = "wasm32", not(target_feature = "atomics")))]
unsafe impl<T> core::marker::Sync for SingletonOnceLazy<T> {}
...
impl<T> Guarded<T> for SingletonOnceLazy<T>
{
    ...
    #[track_caller]
    fn get<'a>(&'a self) -> Self::Guard<'a> { &self.guarded }
    ...
    fn try_get<'a>(&'a self) -> Result<Self::Guard<'a>, Self::Error<'a>> { Ok(&self.guarded) }
}
