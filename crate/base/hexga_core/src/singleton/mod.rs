use super::*;
use crate::{
    cell::{Ref, RefMut, SingleThreadCell, SingleThreadError, SingleThreadMutError},
    identity::Identity,
};
use std::{
...
{
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult { f.write_str("Not init") }
}
...
macro_rules! impl_singleton_methods_get {
    ($singleton_type:ident) => {
        impl<T> $singleton_type<T>
        {
            ...
            #[track_caller]
            #[inline]
            pub fn get<'a>(&'a self) -> <Self as Guarded<T>>::Guard<'a> { Guarded::get(self) }
...
pub trait SingletonOptionable<T>: GuardedMut<T>
{
    ...
    fn take<'a>(&'a self) -> Result<Option<T>, Self::Error<'a>> { self.replace(None) }

    /// Resets the singleton to uninitialized state,
    /// while blocking the current thread until it is able to do so.
    ///
    /// Alias for `take()`.
    fn reset<'a>(&'a self) -> Result<Option<T>, Self::Error<'a>> { self.take() }
}
